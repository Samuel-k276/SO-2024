...
/// A client subscribes to a key.
/// @param key Key to subscribe to.
/// @param pipe_path Path to the notifications pipe.
/// @param notification_pipe_fd File descriptor of the notifications pipe.
/// @return 1 if the subscription was successful, 1 otherwise.
int kvs_subscribe(const char *key, const char *pipe_path, int notification_pipe_fd);

/// A client unsubscribes from a key.
...
/// Unsubscribes all clients from all keys.
